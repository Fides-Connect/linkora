...
class HubSpokeSearch:
    ...
    @staticmethod
    def search_lite_providers(
        search_request: dict[str, Any] | str,
        tenant_id: str,
        limit: int = 10,
        alpha: float = 0.5,
        hyde_text: str = "",
    ) -> list[dict[str, Any]]:
        """Hybrid search inside an isolated ``LiteCompetence`` tenant partition.

        All objects in the tenant belong to the current search — no ownership
        filters (``is_service_provider``, ``last_sign_in``) are needed because
        every record is a Google Places result ingested moments ago.

        Args:
            search_request: Structured query dict (or JSON string) with keys
                ``category``, ``criterions``.
            tenant_id: The tenant that was created by the GP ingestion phase.
            limit: Maximum results to return.
            alpha: Hybrid weight (0=pure vector, 1=pure BM25, 0.5=balanced).
            hyde_text: Hypothetical provider profile for the vector component.

        Returns:
            Provider dicts in the standard result format expected by
            ``ConversationService``.
        """
        import json as _json
        from ai_assistant.hub_spoke_schema import get_lite_competence_collection

        try:
            lite_col = get_lite_competence_collection()
            tenant_col = lite_col.with_tenant(tenant_id)

            # Build query text from structured search_request
            if isinstance(search_request, str):
                try:
                    req = _json.loads(search_request)
                except Exception:
                    req = {}
            else:
                req = search_request
            ...
            criterions = req.get("criterions") or []
            clean_criterions = [c.strip() for c in criterions if c and isinstance(c, str) and c.strip()]
            if category and clean_criterions:
                structured_query_text = f"{category}. Features: {', '.join(clean_criterions)}"
            elif category:
                structured_query_text = category
            elif clean_criterions:
                structured_query_text = ", ".join(clean_criterions)
            else:
                structured_query_text = "service provider"

            query_text = hyde_text.strip() if hyde_text and hyde_text.strip() else structured_query_text

            response = tenant_col.query.hybrid(
                query=query_text,
                limit=limit,
                alpha=alpha,
            ...
            for obj in response.objects:
                ...
                result: dict[str, Any] = {
                    ...
                    "search_optimized_summary": props.get("search_optimized_summary", ""),
                    "primary_type": props.get("primary_type", ""),
                    "category": props.get("category", ""),
                    "skills_list": props.get("skills_list") or [],
                    "review_snippets": props.get("review_snippets") or [],
                    "user": {
                        "uuid": str(obj.uuid),
                        "user_id": None,
                        "name": props.get("name", ""),
                        ...
                        "average_rating": props.get("average_rating", 0.0),
                        "rating_count": props.get("rating_count", 0),
            ...
            logger.info(
                "Lite search [tenant=%s]: %d results for query=%r",
                tenant_id, len(results), query_text[:80],
            )
            return results

        except Exception as exc:
            logger.error(
                "Error in search_lite_providers [tenant=%s]: %s", tenant_id, exc, exc_info=True
            )
            err_lower = (type(exc).__name__ + " " + str(exc)).lower()
            _conn_hints = ("connect", "timeout", "unavailable", "refused", "unreachable")
            if any(h in err_lower for h in _conn_hints):
    ...
    @staticmethod
    def delete_lite_tenant(tenant_id: str) -> None:
        """Delete a ``LiteCompetence`` tenant partition after a search is complete.

        Removing the tenant atomically deletes all objects it contains.
        Logs a warning and returns silently on any error (non-fatal).
        """
        from ai_assistant.hub_spoke_schema import get_lite_competence_collection
        try:
            lite_col = get_lite_competence_collection()
            lite_col.tenants.remove([tenant_id])
            logger.info("Deleted lite tenant %r", tenant_id)
        except Exception as exc:
            logger.warning("Failed to delete lite tenant %r (non-fatal): %s", tenant_id, exc)
