...
def write_env_file(token, env_path=".env"):
    with open(env_path, "w") as f:
        f.write(f"OAUTH_ACCESS_TOKEN={token}\n")
...
if __name__ == "__main__":
    ...
    write_env_file(token, env_file_path)
    print(f"OAUTH_ACCESS_TOKEN written to {env_file_path}:\n{token}")